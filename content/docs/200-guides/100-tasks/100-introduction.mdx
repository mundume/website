...
```rust {6} name="simple_fn" mode="inline"
#[derive(Clone)]
struct State {
...
```rust {3,5,7,9} name="complex_fn" mode="inline"
...
For more advanced scenarios requiring fine-grained control over request processing, you can implement the [`tower::Service`](https://docs.rs/tower-service/latest/tower_service/trait.Service.html) trait directly. This approach accepts `Request<Args, Context>` and provides maximum flexibility.
...
```rust {14} name="service_fn" mode="inline"
#[derive(Clone)]
