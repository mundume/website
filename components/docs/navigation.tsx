...
const generateDocsNavigation = ({
  ...
  return pages
    .filter(
      (page) =>
        page.pathSegments.length === level + 1 &&
        page.urlPath.startsWith(`/docs/${parents.join("/")}`)
    )
    .sort(
      (pageA, pageB) =>
        pageA.pathSegments[level].order - pageB.pathSegments[level].order
    )
    .map((page) => ({
      title: page.navTitle ?? page.title,
      urlPath: page.urlPath,
      collapsible: page.collapsible ?? false,
      children: generateDocsNavigation({
        pages,
        parents: page.pathSegments.map(
          (segment: PathSegment) => segment.pathName
        )
      })
    }))
}
