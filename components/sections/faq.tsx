...
const FAQItem = ({ question, answer, isOpen, onToggle }: FAQItemProps) => {
  ...
  return (
    <div
      className={`w-full bg-[rgba(231,236,235,0.08)] shadow-[0px_2px_4px_rgba(0,0,0,0.16)] overflow-hidden rounded-[10px] outline outline-1 outline-border -outline-offset-1 transition-all duration-500 ease-out cursor-pointer`}
      onClick={handleClick}
    >
      <div className="w-full px-5 py-[18px] pr-4 flex justify-between items-center gap-5 text-left transition-all duration-300 ease-out">
        <div className="flex-1 text-foreground text-base font-medium leading-6 wrap-break-word">{question}</div>
      ...
      >
        ...
        >
          <div className="text-foreground/80 text-sm font-normal leading-6 wrap-break-word">{answer}</div>
        </div>
      </div>
    </div>
  )
